...
const form = reactive({
    type: 'Full-Time', //match with one of the choice present in select option
    title: '',
    description: '',
    salary: '',
    location: '',
...
const handleSubmit = async () => {
    const updateJob = {
        ...
        company: {
            ...
            contactEmail: form.company.contactPhone,
    ...
    try {
        ...
        console.error('Error push job data', error);
        toast.error('Job was not Updated');
    }
}
onMounted(async () => {
    try {
        const response = await axios.get(`/api/jobs/${jobId}`); //json object not the url path
        state.job = response.data;
        // Populata inputs with mathed id data's
